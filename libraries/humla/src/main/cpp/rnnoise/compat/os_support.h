 ...
 * any release tag: it drops the erroneous includes and rewrites the three
 ...
 * v0.2 we put this header on the include path, which makes those call sites
 * resolve to exactly the macro upstream's fix uses.  `RNN_CLEAR` is
