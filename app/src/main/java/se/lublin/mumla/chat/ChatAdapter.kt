 ...
 *
 * **The log is bounded, and so is what this holds.** `MumlaService` keeps at most
 * `ChatMessageLog.MAX_ENTRIES` (500) messages and drops the oldest first; the trimming happens
 * there, not in this class. A dropped message leaves the next submitted list at its head, which
 * is the delete case above. Every parsed body is cached on its message while the message is in
 * the log: a retained-heap delta with a forced GC put this at ~492 bytes of `Spanned` per message,
 * i.e. roughly 240 KiB at the cap. That is a reported figure, not an assertion — it was never
 * checked against an instrumented measurement and no test holds it — but the shape it describes
 * is real: the cache grows with the log, up to the cap. It sits on top of the log itself and on top
 * of `AsyncListDiffer`'s two list references. Each [submitMessages] also copies the whole log and
 * re-walks it once — O(N) per arriving message with N at most 500 — though only the copy is
 * charged to the main thread (measured 0.3 us at 200 messages, 7-15 us at 5 000); the walk runs on
 * [parseDispatcher]. The 5 000 and 20 000 figures in this comment are measurements beyond the cap,
 * kept for scale; production no longer reaches them.
 *
